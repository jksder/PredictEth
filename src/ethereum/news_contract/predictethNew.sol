...
contract RandomNumber{
    ...
    //constructor: deployer of contract is admin
    constructor(){
        admin = msg.sender;
    }
    ...
    //modifier that enforces admin use only
    modifier adminOnly(){
        require(msg.sender==admin,"Admin only function");
        _;
    }
    
    //modifier that enforces player use only
    modifier playerOnly(){
        require(msg.sender!=admin,"Player only function");
        _;
    }
    
    //modifier that enforces player presence in event
    modifier playerFound(string memory eventHash, address payable winner){
        require(admin!=winner, "Admin cannot send themselves funds"); //the admin cannot send themselves the money
    ...
    function addEvent (string memory eventHash, string memory title) payable public adminOnly{
        //the 
        Event memory e;
        e.title=title;
        e.pool+=msg.value; 
    ...
    function enterEvent(string memory eventHash) payable public playerOnly playerNew(eventHash){ 
    ...
    function endEvent(string memory eventHash, address payable winner) public adminOnly playerFound(eventHash, winner){
